...
namespace TrTrestAddin_MK.Windows
{
    ...
    -                                   Задачи
        ...
        * 19. -- Перенос часть таблицы после 8-го образца (Еще раз уточнить номер образца), можно создать новую спецификацию после n-ой ограждений -- 
             ...
             19.4 --- Перенос после определенной высоты, а не после 8-го образца, можно считать высоту каждой строке, нужна определенная высота (475 мм пока что с учетом новой заголовки (имя спецификации)), 
                        ...
                        Сделать регулируемую высоту для переноса ->
    ...
    public partial class Struct_MetalRollingForm : Form
    {
        ...
        public MR_ExternalEventClass myExternalClass;
        public ExternalEvent ExEvent;

        public int number;
        public List<string> strings = new List<string>();

        public Struct_MetalRollingForm(ExternalCommandData commandData)
        {
            InitializeComponent();
            ...
            myExternalClass = new MR_ExternalEventClass(commandData, strings, number);
        ...
        private void OK_Click(object sender, EventArgs e)
        {
            ...
            TaskDialog.Show("t", number.ToString());
            strings.Add("a");
            strings.Add("b");
            //returned(number);
            ExEvent.Raise();
        ...
        {
            TaskDialog.Show("t", "returend: " + num.ToString());
        }
    ...
    public class MR_ExternalEventClass : IExternalEventHandler   // Реализую интерфейс обработчика внешнего события
    {
        UIDocument uidoc;
        Document doc;
        ExternalCommandData cmdata;

        int scheduleHeight_got;
        List<string> strings_Got;

        public void print()
        {
            //TaskDialog.Show("t", "strings: " + strings.Count.ToString());
        }
        public MR_ExternalEventClass(ExternalCommandData commandData, List<string> strings, int scheduleHeight)
        {
            //int a = frm.number;
            //TaskDialog.Show("t", "frm.number: " + frm.number.ToString());
            cmdata = commandData;
            uidoc = commandData.Application.ActiveUIDocument;
            doc = uidoc.Document;
            //TaskDialog.Show("t", "strings: " + strings.Count.ToString());
            //TaskDialog.Show("t", "scheduleHeight: " + scheduleHeight.ToString());
            scheduleHeight_got = scheduleHeight;
            strings_Got = strings;
            //TaskDialog.Show("t", "strings_Got: " + strings_Got.Count.ToString());
            //TaskDialog.Show("t", "scheduleHeight_got: " + scheduleHeight_got.ToString());
        }
        ...
        public void Execute(UIApplication app)
        {
            ...
            Struct_MetalRollingForm frm_2 = new Struct_MetalRollingForm(cmdata);
            int num = frm_2.number;
            TaskDialog.Show("t", "Execute method scheduleHeight_got : " + num.ToString());
            TaskDialog.Show("t", "Execute method strings_Got: " + strings_Got.Count.ToString());
            TaskDialog.Show("t", "Execute method strings_Got: " + String.Join("\n", strings_Got));
            ...
            //        RowCountDeterMine(doc, all_Fences_Instances_Lojiya, ref listofList_FencesExamples_Lojiya_Distinct, ref listOf_Matrix_Lojiya_RowCount, scheduleHeight_got);
            ...
            //        RowCountDeterMine(doc, all_Fences_Instances_Krovlya, ref listofList_FencesExamples_Krovlya_Distinct, ref listOf_Matrix_Krovlya_RowCount, scheduleHeight_got);
            ...
            //        RowCountDeterMine(doc, all_Fences_Instances_Poruchen, ref listofList_FencesExamples_Poruchen_Distinct, ref listOf_Matrix_Poruchen_RowCount, scheduleHeight_got);
            ...
            //        RowCountDeterMine(doc, all_Fences_Instances_Met_Reshetka, ref listofList_FencesExamples_Met_Reshetka_Distinct, ref listOf_Matrix_Met_Reshetka_RowCount, scheduleHeight_got);
        ...
        static void RowCountDeterMine(Document doc, List<FamilyInstance> all_Fences_Instances, ref List<List<FamilyInstance>> listofList_FencesExamples_Distinct, ref List<int> listOf_Matrix_RowCount, int scheduleHeight)
        {
        ...
        static void BeforeArray2DFilling(List<List<FamilyInstance>> listofList_FencesExamples_Distinct,
            ref List<List<List<FamilyInstance>>> ListOfListsOfLists_fencesExamples, ref List<List<List<int>>> ListOfListsOfLists_FExamples_eachDetail_Amount, ref List<List<double>> ListOfLists_FExamples_Weight_Amount, Document doc)
        {
        ...
        public static void ScheduleFilling(ViewSchedule vs, string[,] Matrix, string[] Arr_sech_prof, string[] Arr_mat_and_mat_Oboznach, string vs_Name, Document doc, ElementId categoryId, TextNote textNote)
        {
            if (null != vs)
            {
