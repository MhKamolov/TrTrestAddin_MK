...
{
    public partial class AR_RoomDecorationForm : Form
    {
        /*                               Начать с
                                       3 - задачи   
      ...
      -                                   Задачи
        ...
        */
        ...
        public AR_RoomDecorationForm(ExternalCommandData commandData)
        {
            InitializeComponent();
        ...
        private void RDecor_Form_Load(object sender, EventArgs e)
        {
    ...
    public class ExternalEventClass : IExternalEventHandler   // Реализую интерфейс обработчика внешнего события
    {
        ...
        public void Execute(UIApplication app)
        {
            ...
            else
            {
                ...
                if (null == wallType)
                {
                    throw new Exception("WallType == null");
                }
                ...
                foreach (Room room in listOfRooms_Got)
                {
                    ...
                    foreach (List<BoundarySegment> boundarySegments_list in room.GetBoundarySegments(opt).ToList())
                    {
                        ...
                        {
                            if (doc.GetElement(item.ElementId) != null /*&& doc.GetElement(item.ElementId).Category.Name == "Стены"*/) // 11:49 (13.02)
                            {
                    ...
                    using (Transaction tx = new Transaction(doc, "Test"))
                    {
                        ...
                        for (int i = 0; i < boundarySegments_list_Final.Count; i++)
                        {
                            ...
                            if (doc.GetElement(boundarySegments_list_Final[i].ElementId).Category.Name == "Стены")
                            {
                                ...
                                {
                                    // double iWallWidth = (doc.GetElement(boundarySegments_list_Final[i].ElementId) as Wall).Width;
                                    //double jWallWidth = (doc.GetElement(boundarySegments_list_Final[j].ElementId) as Wall).Width;
                                    //if (boundarySegments_list_Final[i].GetCurve().CreateOffset(iWallWidth * (-1) / 2, new XYZ(0,0,1)).GetEndPoint(1) ==
                                    //    boundarySegments_list_Final[j].GetCurve().CreateOffset(iWallWidth * (-1) / 2, new XYZ(0, 0, 1)).GetEndPoint(0))
                                    //{
                                    //    (boundarySegments_list_Final[i].GetCurve().GetEndPoint(1).X == boundarySegments_list_Final[j].GetCurve().GetEndPoint(0).X ||
                                    //        boundarySegments_list_Final[i].GetCurve().GetEndPoint(1).Y == boundarySegments_list_Final[j].GetCurve().GetEndPoint(0).Y)
                                    //}
                                    //strList.Add((boundarySegments_list_Final[i].GetCurve().CreateOffset(iWallWidth * (-1) / 2, new XYZ(0, 0, 1)).GetEndPoint(1)).ToString());

                                    //if (boundarySegments_list_Final[i].GetCurve().GetEndPoint(1).X != boundarySegments_list_Final[j].GetCurve().GetEndPoint(0).X ||
                                    //    boundarySegments_list_Final[i].GetCurve().GetEndPoint(1).Y != boundarySegments_list_Final[j].GetCurve().GetEndPoint(0).Y)
                                    //{

                                    //}
                                    /*else*/
                                    if (doc.GetElement(boundarySegments_list_Final[j].ElementId).Category.Name == "Стены"
                               //&& (boundarySegments_list_Final[i].GetCurve().CreateOffset(iWallWidth * (-1) / 2, new XYZ(0, 0, 1)).GetEndPoint(1).Y ==
                               //boundarySegments_list_Final[j].GetCurve().CreateOffset(jWallWidth * (-1) / 2, new XYZ(0, 0, 1)).GetEndPoint(0).Y)
                               && (boundarySegments_list_Final[i].GetCurve().GetEndPoint(1) == boundarySegments_list_Final[j].GetCurve().GetEndPoint(0))
                               && ((boundarySegments_list_Final[i].GetCurve() as Line).Direction.Y == (boundarySegments_list_Final[j].GetCurve() as Line).Direction.Y ||
