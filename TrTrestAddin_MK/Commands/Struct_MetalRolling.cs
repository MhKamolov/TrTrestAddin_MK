...
namespace TrTrestAddin_MK.Commands
{
    ...
    -                                   Задачи
        ...
        * 16. Создаем программно вид спецификаций для каждой семейств ограждений (пока 4), можно сделать динамический от количество разных семейств (например в случае кровли) -> Выполнена (пока не такая динамичня) *** !!! уточнить
        ...
        * 24. - Сделать отдельных спецификаций если ограждений кровли имееют разную "группа модели" (как будто это другие ограждений на уровне лоджий, поручень и т.д.) -- Сделать после рефакторинга, ограждения будут динамично сгруппироваться по Группу модели
                таким образом, когда добавляется новая семейство ограждение, не надо будет для него отдельно написать код - 
    ...
    ##############################################################################################################################################################################################################
                                              ...
                                              Вопросы
        * 1. По каким параметром проверить различие между составляющими элемантами? я пока проверяю по Name и иногда по Id. Думаю, что есть идеи получше?  -> Ответ: По параматру "ADSK_Наименование" в типоразмерах элементов
        * 2. Уточнить момент с различными спецификациями, и необходимости ручного регулирование их контента (все что ниже основной заголовки)?
        * 3. Обсудить эффективность параметров по каторым делается проверка, выборка и т.д.
    ...
    public class Struct_MetalRolling : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            using (Transaction tx = new Transaction(doc))
            {
                ...
                .Where(el => el.Category.Name == "Ограждение")
                ...
                if (all_Fences_Instances_Lojiya.Count != 0)
                {
                    ...
                    RowCountDeterMine(doc, all_Fences_Instances_Lojiya, ref listofList_FencesExamples_Lojiya_Distinct, ref listOf_Matrix_Lojiya_RowCount);
                ...
                if (all_Fences_Instances_Krovlya.Count != 0)
                {
                    ...
                    RowCountDeterMine(doc, all_Fences_Instances_Krovlya, ref listofList_FencesExamples_Krovlya_Distinct, ref listOf_Matrix_Krovlya_RowCount);
                ...
                if (all_Fences_Instances_Poruchen.Count != 0)
                {
                    ...
                    RowCountDeterMine(doc, all_Fences_Instances_Poruchen, ref listofList_FencesExamples_Poruchen_Distinct, ref listOf_Matrix_Poruchen_RowCount);
                ...
                if (all_Fences_Instances_Met_Reshetka.Count != 0)
                {
                    ...
                    RowCountDeterMine(doc, all_Fences_Instances_Met_Reshetka, ref listofList_FencesExamples_Met_Reshetka_Distinct, ref listOf_Matrix_Met_Reshetka_RowCount);
        ...
        static void RowCountDeterMine(Document doc, List<FamilyInstance> all_Fences_Instances, ref List<List<FamilyInstance>> listofList_FencesExamples_Distinct, ref List<int> listOf_Matrix_RowCount)
        {
            ...
            Document doc)
        {
            ...
            foreach (var item in ViewSchedules)
            {
                if (item.IsValidObject == true && item.Name.Contains(vs_Name) && Convert.ToInt32(new String(item.Name.Where(Char.IsDigit).ToArray())) > ListOfListsOfLists_fencesExamples.Count) // !!! улучшить условие
                {
                    doc.Delete(item.Id);
                }
            }
